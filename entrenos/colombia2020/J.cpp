...
int rmq(const vi& st, int p, int l, int r, int i, int j) {
	if(l > j || r < i) return INF;
	if(l >= i && r <= j) {
		return st[p];
	}

	return min(rmq(st, 2*p, l, (l+r)/2, i, j), rmq(st, 2*p+1, (l+r)/2+1, r, i, j));
	
}

void update(vi& st, int p, int l, int r, int i, int v) {
	if(l > i || r < i) return;
	if(l == i && r == i) {
		st[p] = v;
	}
	else {
		update(st, 2*p, l, (l+r)/2, i, v);
		update(st, 2*p+1, (l+r)/2+1, r, i, v);
...
int segment_tree_query(int x, int y, int j) {
	return rmq(st_col[j], 1, 0, n_nodes-1, x, y);
}
...
int ans_query(int a, int b) {
    ...
    for (; head[a] != head[b]; b = parent[head[b]]) {
        if (depth[head[a]] > depth[head[b]])
            swap(a, b);
        for(int j=0; j < B; ++j) {
	        int cur_heavy_path_max = segment_tree_query(pos[head[b]], pos[b], j);
	        res[j] = min(res[j], cur_heavy_path_max);
	        if(res[j] == 0) return -1;
	    }
    }
    if (depth[a] > depth[b])
        swap(a, b);
    for(int j=0; j < B; ++j) {
	    int last_heavy_path_max = segment_tree_query(pos[a], pos[b], j);
	    res[j] = min(res[j], last_heavy_path_max);
	}
	int ans = 0;
	for(int j=0; j < B; ++j) {
		if(res[j] == 2) ans++;
...
int main() {
	...
	*/
    ...
    for(int i=0; i < n+q; ++i) {
    	...
    	query e = queries[i];
    	if(e.type == 0) {
    		for(int j=0; j < b; ++j) update(st_col[j], 1, 0, n_nodes-1, pos[realidx[e.x]], 0);
    	}
    	else if(e.type == 2) {
    		int topics = 0;
    		...
    		for(int j=0; j < b; ++j) {
    			update(st_col[j], 1, 0, n_nodes-1, pos[realidx[e.x]], 1+((topics&(1<<j)) ? 1 : 0));
    		}
    	}
    	else {
    		int x = realidx[e.x];
